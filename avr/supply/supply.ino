...
#define PRIMARY_R1         3
#define PRIMARY_R2         3
#define PRIMARY_DIOD_V_X10 6
#define PRIMARY_MAX_LEVEL  144

#define RESERVE_R1         2
#define RESERVE_R2         2
#define RESERVE_DIOD_V_X10 6
#define RESERVE_MAX_LEVEL  120

#define RESERVE_5V_R1         2
#define RESERVE_5V_R2         2
#define RESERVE_5V_DIOD_V_X10 6
#define RESERVE_5V_MAX_LEVEL  120
...
void loop() {
  ...
  // write data
  readAndSendBatteryLevel(PRIMARY_BATTERY_LEVEL, PRIMARY_R1,    PRIMARY_R2,    PRIMARY_DIOD_V_X10,    PRIMARY_MAX_LEVEL);
  readAndSendBatteryLevel(RESERVE_BATTERY_LEVEL, RESERVE_R1,    RESERVE_R2,    RESERVE_DIOD_V_X10,    RESERVE_MAX_LEVEL);
  readAndSendBatteryLevel(RESERVE_5V_LEVEL,      RESERVE_5V_R1, RESERVE_5V_R2, RESERVE_5V_DIOD_V_X10, RESERVE_5V_MAX_LEVEL);
...
void readAndSendBatteryLevel(analog_pin_t pin, uint16_t r1, uint16_t r2, uint8_t diod_x10, uint8_t maxLevel) {
  uint8_t level = (analogRead(pin) * r1 * 5 / 1023 / (r1 + r2)) + diod_x10;
  crc += level;
  Serial.write(level);
  level = ((uint16_t)level * 100) / maxLevel;
  crc += level;
  Serial.write(level);
}
